...
public static class MentorshipEndpoints
{
    ...
    private static List<MentorExpertiseArea>? ParseExpertiseAreas(string? raw)
    {
        ...
        var parsed = raw
            ...
            .Where(part => Enum.TryParse<MentorExpertiseArea>(part, ignoreCase: true, out _) &&
                           Enum.IsDefined(Enum.Parse<MentorExpertiseArea>(part, ignoreCase: true)))
            .Select(part => Enum.Parse<MentorExpertiseArea>(part, ignoreCase: true))
            .ToList();
