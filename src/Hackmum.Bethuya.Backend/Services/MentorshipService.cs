    ...
    CommunityJourneyReadModelService journeyReadModelService)
{
        ...
        CancellationToken ct = default)
    {
        ...
        // Only clear discoverability when pausing or opting out; preserve the stored consent on re-opt-in.
        if (request.Status is MentorshipStatus.Paused or MentorshipStatus.OptedOut)
        {
            existing.IsDiscoverable = false;
        }
