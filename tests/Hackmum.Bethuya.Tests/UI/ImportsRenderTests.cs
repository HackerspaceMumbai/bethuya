...
public class ImportsRenderTests
{
    ...
    public async Task StartDryRun_FailedReupload_ClearsStaleBatchAndPreviewFromPriorSuccess()
    {
        ...
        // Now invoke StartDryRunAsync directly with no file selected (_selectedFile is null),
        // simulating any failed/invalid new attempt. The prior batch/preview must be cleared
        // immediately, rather than left rendered alongside a now-stale "Commit import" button
        // that would submit the OLD batch id.
