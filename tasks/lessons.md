...
- **Fix:** Replaced the `<a href>` elements with native `<button type="button">` elements wired to a `SelectPersona(DevelopmentPersona persona)` handler that calls `Navigation.NavigateTo(url, forceLoad: true)`. This matches the repo's own established pattern in `SocialProfileConnections.razor`. Verified end-to-end via Playwright: the browser console log shows exactly one additional `_blazor` WebSocket circuit connection per persona switch (a genuine new page load), and the SSR-rendered `active-persona`/`active-persona-role` correctly reflect the new persona both immediately after the switch and after an explicit hard reload.
- **Prevention:** Any interactive element in a Blazor Server/Web-App component that must trigger a server-side side effect requiring a fresh circuit (auth cookie changes, session resets, anything the current circuit's cached state must not silently absorb) should use `NavigationManager.NavigateTo(url, forceLoad: true)` from a button/handler, never a bare `<a href>`. Confirm the fix isn't a stale-build artifact by rebuilding the target project **and** restarting the Aspire resource together before re-testing — a stale build or a not-yet-restarted resource produced misleading "still broken" results twice during this investigation.

## [2026-08-04] bUnit's `FakeNavigationManager` does not throw `NavigationException` for `forceLoad: true`; assert on `.Uri` instead
- **What happened:** A bUnit test asserting the persona-switch button triggers a forced navigation initially expected `Assert.Throws<NavigationException>(...)`, following the documented behavior of the real `NavigationManager.NavigateTo(url, forceLoad: true)` (which throws `NavigationException` when there is no host to actually perform the browser navigation, e.g., outside a real browser). In bUnit 1.40.0, this exception was never thrown; the test failed because no exception occurred.
- **Root cause:** bUnit's `FakeNavigationManager` — used automatically for any component under test via `TestContext` — does not replicate the real `NavigationManager`'s `NavigationException`-throwing behavior for `forceLoad: true`. Instead, it silently accepts the navigation and updates its own `.Uri` property in-memory, identical to a non-forceLoad navigation. This is a divergence between the fake test double and the real ASP.NET Core Blazor `NavigationManager`, not a bug in the component under test. (Confirmed the real property is `Location`, not `Uri`, via reflection against the shared-framework `Microsoft.AspNetCore.Components.dll` — but this became moot once it was clear bUnit never throws it here.)
- **Fix:** Rewrote the test to resolve `ctx.Services.GetRequiredService<FakeNavigationManager>()`, click the persona button, and assert the expected target URL appears in `navigationManager.Uri` — verifying intent (the component requested the right forceLoad navigation) without depending on bUnit's incomplete fidelity to the exception-throwing real implementation.
- **Prevention:** When testing `NavigateTo(..., forceLoad: true)` calls in bUnit, do not assume `NavigationException` will be thrown — check `FakeNavigationManager`'s actual behavior for the bUnit version in use first, and prefer asserting on the resulting `.Uri`/`.History` state, which is stable across bUnit versions.

## [2026-08-04] Top-level `[Authorize]`-decorated Blazor Server pages return a bare HTTP 403 on denial, not a client-rendered "not authorized" page
- **What happened:** The Layer 3 E2E test initially assumed that navigating to an organizer/curator-gated page (`/curation/{eventId}`, `@attribute [Authorize(Policy = RequireOrganizerOrCurator)]`) as an unauthorized persona (Farah, Attendee-only) would return an HTTP 200 page containing Blazor's client-rendered `<AuthorizeRouteView><NotAuthorized>` fallback text ("You are not authorized to view this page.", as defined in `Routes.razor`). Direct HTTP probing (`Invoke-WebRequest` with a manually attached persona cookie) and the Playwright E2E run both showed the server instead returns a genuine, empty-bodied **HTTP 403 Forbidden**.
- **Root cause:** `CurationView.razor` is mapped as a top-level routable component via `app.MapRazorComponents<App>().AddInteractiveServerRenderMode()` (no `.RequireAuthorization()` chained). Because the page component itself carries `@attribute [Authorize(Policy = ...)]`, Blazor's endpoint-to-component mapping surfaces that attribute as ASP.NET Core endpoint metadata. On a genuine full-page navigation (not Blazor's in-circuit enhanced navigation), `UseAuthorization()` middleware evaluates that metadata *before* the Razor Components pipeline ever runs, and on policy failure invokes the current authentication scheme's `ForbidAsync`. The repo's custom `DevelopmentAuthenticationHandler` doesn't override forbid behavior, so the default `AuthenticationHandler<TOptions>.HandleForbiddenAsync` just sets a bare 403 status with no body — this happens at the ASP.NET Core middleware tier, entirely bypassing Blazor's own `AuthorizeRouteView`/`<NotAuthorized>` client-rendered fallback, which only ever gets a chance to run for in-circuit (non-full-reload) navigations.
- **Fix:** This is the app's genuine, pre-existing, by-design behavior for `[Authorize]`-decorated top-level Blazor Server pages (not a regression introduced by Layer 3, and not something Layer 3 should "fix" — changing it would require chaining `.RequireAuthorization()` differently or adding a custom `IAuthorizationMiddlewareResultHandler`, both out of Layer 3's scope). The E2E test (`DevPersonaAuthorizationFlowTests.cs`) was corrected to assert `GotoWithBudgetAsync(...)`'s returned `IResponse.Status == 403` for Farah and `== 200` for Vikram, rather than asserting on rendered page text — this still fully proves the "organizer-gated UI/API outcome is denied" requirement, just via the mechanism the app actually uses.
- **Prevention:** When writing E2E assertions against Blazor Server pages decorated with a top-level `[Authorize(Policy = ...)]` attribute, verify the actual HTTP-level denial behavior (via a direct `Invoke-WebRequest`/curl probe with the relevant auth cookie/header) before writing text-based UI assertions — a full-page navigation to such a route can be denied entirely at the ASP.NET Core middleware tier, never reaching Blazor's client-rendered `<NotAuthorized>` fallback, even though that fallback exists and is correctly wired for in-circuit navigation denials.

## [2026-08-04] DI-composition gating verified in bUnit; AuthorizeView cascades correctly from a sibling root island
- **What happened:** Layer 3 (`DevPersonaToolbar`) gates visibility via a DI marker
  (`IDevPersonaToolbarFeature`, registered only when `Provider == None && IsDevelopment()`)
  rather than relying solely on cosmetic `@if`. There were two open risks going into bUnit
  testing: (1) whether `<AuthorizeView>` would correctly resolve `AuthenticationState` when the
  component is mounted as its own top-level root in `App.razor` (a sibling to `<Routes>`, not
  nested inside it), and (2) whether the Blazor Blueprint components used (`BbCard`,
  `BbCardHeader`, `BbCardTitle`, `BbCardDescription`, `BbCardContent`, `BbSeparator`, `BbButton`)
  would accept the parameters used, since BB components throw at runtime (not compile time) on
  unknown parameters.
- **Root cause / verification:** Both were empirically resolved by `DevPersonaToolbarRenderTests.cs`
  (TUnit + bUnit): `ctx.AddTestAuthorization().SetAuthorized(name).SetRoles([...]).SetClaims(new
  Claim("name", ...), new Claim("email", ...), new Claim("sub", ...))` correctly flows into
  `<AuthorizeView><Authorized>` even for a component rendered standalone via
  `ctx.RenderComponent<DevPersonaToolbar>()` (not nested under `<Routes>`) — bUnit's
  `TestAuthorizationContext` cascades `AuthenticationState` globally through the DI-registered
  `AuthenticationStateProvider`, not through component nesting. All 5 tests rendered the full BB
  card markup (default `_isExpanded = true`) without exception, confirming the parameter names
  inferred from DLL string-search + existing `.razor` usage patterns were correct.
- **Fix:** No code changes were needed — the original design (plain `<AuthorizeView>`, no
  `IBethuyaAuthStateProvider` indirection) was correct. This also matches the existing
  `LoginDisplay.razor` precedent, which lives in the *shared* RCL and also uses `<AuthorizeView>`
  directly, despite Trinity's charter listing `IBethuyaAuthStateProvider` as a "Hybrid safety"
  constraint — that constraint applies to Hybrid-specific storage/offline concerns, not to reading
  the ambient `AuthenticationState`, which is standard Blazor and host-agnostic.
- **Prevention:** For any future Web-only or shared-RCL component that needs to display the
  authenticated identity, prefer plain `<AuthorizeView>` (matches established precedent) and
  verify DI-composition gates + BB component parameter safety together in one bUnit test file: a
  "feature registered" happy-path test (exercises the full BB markup) and a "feature not
  registered" negative test (asserts empty markup), using `ctx.Services.AddSingleton<TMarker,
  TImpl>()` presence/absence as the switch.
