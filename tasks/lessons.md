...
## [2026-07-02] PR3 default-deny: referenced app appsettings.json pollutes test config
- **What happened:** After flipping the authenticated FallbackPolicy default to ON, two kinds of auth tests failed: the new default-deny tests saw the fallback as OFF, and 11 `DevelopmentAuthenticationTests` profile tests failed with body-inference errors.
- **Root cause:** (1) The test project references both Backend and Web app projects; their `appsettings.json` files flow into the test output and one wins. The Web `appsettings.json` (which PR3 sets `Authorization:EnforceAuthenticatedFallback=false` to keep public Blazor pages working) landed in the test bin, so `Host.CreateApplicationBuilder` / `WebApplication.CreateBuilder` in tests read `false` and masked the new production default. (2) Replacing hand-rolled `ClaimsPrincipal` reads in `ProfileEndpoints`/`RegistrationEndpoints`/`PlanningCycleEndpoints` with injected `IUserContext` made every minimal test host that maps those endpoints require `IUserContext` in DI; hosts lacking `AddBethuyaUserContext()` failed RDF inference (treated `IUserContext` as a request body).
- **Fix:** (1) Auth test hosts (`HeaderRoleAuthHost`, `AuthorizationFallbackPolicyTests.CreateBuilder`) now call `builder.Configuration.Sources.Clear()` before adding explicit in-memory config, making default-deny semantics deterministic regardless of ambient files. (2) Registered `AddBethuyaUserContext()` in the affected test hosts (`DevelopmentAuthenticationTests.CreateBuilderWithProviderNone`, `PlanningCycleEndpointValidationTests`).
- **Prevention:** Auth/config unit tests that assert a code default must not rely on ambient `appsettings.json` — clear config sources or use an isolated builder. When adding a new DI-injected handler parameter (e.g. `IUserContext`), register it in every test host that maps the endpoint, or RDF will misclassify it as a body.
...
## [2026-07-01] PR5 — Observability test isolation + LoggerMessage/DI gotchas

- **CA1873 (expensive argument in skipped log call):** Passing a method call (e.g. `auditEvent.Decision.ToString()`) directly as an argument to a `LoggerMessage` source-gen method trips CA1873 under `TreatWarningsAsErrors`. Fix: precompute into a local (`var decision = ...;`) and pass the local.
- **Global Meter/ActivitySource names cross-contaminate parallel TUnit tests:** A `MeterListener` matching by meter *name* and an `ActivityListener` matching by *source name* capture measurements/spans emitted by other concurrently-running tests' hosts (TUnit runs in parallel by default), making `.Single()` assertions flaky. Fixes: (1) scope `MeterListener` to a specific `Meter` *instance* via reference equality on `instrument.Meter` (added internal `AuthorizationMetrics.Meter` accessor + `InternalsVisibleTo`); for middleware tests resolve the host's singleton `AuthorizationMetrics` from `app.Services`. (2) The `ActivitySource` is process-global (one static instance shared by all auditors), so instance-scoping is impossible — instead filter captured activities by a per-test-unique tag (e.g. a GUID subject-hash sentinel, or the test's distinct subject hash).
- **DI last-registration-wins lets tests substitute the auditor:** `AddBethuyaAuthorization` uses `TryAddSingleton` for `IAuthorizationAuditor`/`AuthorizationMetrics`; test hosts that later call `AddSingleton` in `configureServices` win, so a `RecordingAuditor` can replace the real one without touching production wiring.
- **Middleware-result handler is Backend-only:** `IAuthorizationMiddlewareResultHandler` (the 401/403 outcome recorder) is registered via a separate `AddBethuyaAuthorizationObservability` called only from Backend `Program.cs`, never from `AddBethuyaAuthorization`, to avoid any Blazor/Web regression. Tests that need it register it manually in `configureServices`.
- **New service-typed handler params break metadata-inference hosts:** Adding `IAuthorizationAuditor` to endpoint handler signatures made `RouteGroupAuthorizationTests` fail with "Failure to infer one or more parameters" until `IAuthorizationAuditor` was added to that host's stub service list — same lesson as PR4's co-mapped-deps rule.
