...
- **Status:** done
- **Completion Evidence:** 
  - Branch: `indcoder-community-acceptance-harness`
  - All 25 TUnit integration tests pass (runtime: 45s 682ms)
  - 7 Playwright E2E tests compile successfully (runtime validated with live Aspire)
  - Build: `dotnet build Bethuya.slnx` — 0 errors, 0 warnings
  - Code review gate: Passed (4 issues fixed: null coalescing, execution order, duplication, doc)
  - Perf review gate: Passed (8 issues fixed: 6 HttpClient double-dispose, 1 StringComparison, rest deferred as info)
  - All critical and moderate findings resolved
  - PR targeting `indcoder-community-simulation-seeder` (PR #55) — **ready for submission**
...
- **Surfaces:**
  ...
  2. `tests/Bethuya.IntegrationTests/CommunityAcceptanceHarnessTests.cs` — TUnit acceptance tests (14 tests across 5 groups): deterministic seeding (all 6 personas, stable IDs), idempotency (zero new rows on reseed), persona persistence through existing APIs (journey/dashboard), authorization boundaries (Farah→403, Vikram→200), decision audit attribution (identity endpoint proves persona email), and structured log placeholders.
  3. `tests/Hackmum.Bethuya.E2E/Tests/CommunityAcceptanceHarnessTests.cs` — Playwright E2E acceptance flow (7 tests): homepage/toolbar visibility, persona switching persistence across nav, Vikram access/Farah denial of organizer pages, seeded data visibility, decision creation audit, persona session switching with screenshots.
  4. No new CLI commands or entry points (existing `dotnet test` filter works: `--filter "CommunityAcceptanceHarness*"`).
...
- **Acceptance:** All TUnit tests pass (real Aspire/Postgres); all Playwright E2E tests pass with `BETHUYA_BASE_URL` set; targeted `dotnet test tests/Bethuya.IntegrationTests` + `dotnet test tests/Hackmum.Bethuya.E2E` both green; full `dotnet build Bethuya.slnx` 0 warnings; code-review + dotnet-diag:optimizing-dotnet-performance findings (if any) resolved; Playwright screenshots captured; PR opened targeting `indcoder-community-simulation-seeder` (PR #55), not main.
...
- **Result:** Delivered on branch `indcoder-community-simulation-seeder` (commits `0693d88`, `fa29a9f`, `3722108`) targeting PR #53 (`indcoder-developer-persona-toolbar`). `CommunitySimulationSeeder.cs` idempotently provisions the six catalog personas (CommunityMember + Platform ExternalIdentity + 2-3 differentiated ParticipationLedgerEntry rows each), one shared fixture `Event` (dedup key: `Hashtag="community-simulation-fixture"`), and one `Registration` per persona against it. New endpoint `POST /api/dev/community-simulation/seed` requires `RequireOrganizer` and is Development-only. New AppHost command `seed-community-simulation` (impersonates Vikram via `X-Bethuya-Dev-Persona` header — documented dev-only trust boundary). Code review caught a real gap (no unique DB index backing Registration idempotency) — fixed with migration `20260808123745_RegistrationEventEmailUniqueness` (unique index on `Registration(EventId, Email)`) plus a verified re-query fallback. Final evidence: `dotnet build Bethuya.slnx` 0 warnings/0 errors; 334/334 unit tests passing (`Hackmum.Bethuya.Tests`); 16/16 real Aspire/Postgres integration tests passing (`Bethuya.IntegrationTests`), including seed-twice idempotency, Farah→403/Vikram→200, and passport/dashboard visibility proofs. `seed-curation`/`CurationSampleSeeder`/`/api/dev/curation/seed` left untouched (only a shared guard-comment line was extended to mention the new route). Layers 5/6 not started.
- **Description:** Add a *separate*, explicit development seed path for the six canonical development personas (Anish/Priya/Rohan/Maya/Farah/Vikram from `ServiceDefaults.Auth.DevelopmentPersonaCatalog`) that deterministically and idempotently persists `CommunityMember` + `ExternalIdentity` (Platform provider, subject = persona catalog `Subject`, e.g. `dev-persona-anish`) + a small differentiated `ParticipationLedgerEntry` history + a couple of `Event`/`Registration` rows per persona, reusing existing EF entities/repositories only (no new domain/read models). Must NOT touch/duplicate `seed-curation`/`/api/dev/curation/seed`/`CurationSampleSeeder`, must not duplicate the persona catalog, must not violate the `(CommunityMemberId, Connector, ProvenanceKey)` ledger uniqueness, and must not invent Graph/Chapters/Projects/mentorship-progression/CommunityLead/analytics/federation/SimulationDataset domains (issue #50 v2 out of scope).
- **Surfaces:**
  1. `src/Hackmum.Bethuya.Backend/Services/CommunitySimulationSeeder.cs` — new seeder service (mirrors `CurationSampleSeeder` structural conventions: primary-ctor DI, `[LoggerMessage]` structured summary log, narrow `CommunitySimulationSeedResult` record with created/existing/repaired counts + stable persona keys, no PII dumps).
  2. `src/Hackmum.Bethuya.Backend/Endpoints/DevelopmentEndpoints.cs` — add `POST /api/dev/community-simulation/seed` inside the existing `MapDevelopmentEndpoints` (Development-only guarded group), with `.RequireAuthorization(BethuyaPolicyNames.RequireOrganizer)` (new — the legacy curation seed route stays exactly as-is, unauthenticated, unchanged).
  3. `AppHost/AppHost/Commands/SeedCommandExtensions.cs` — add a new `seed-community-simulation` command alongside (not replacing) `seed-curation`, POSTing to the new endpoint. AppHost is Neo's exclusive-lock area per routing.md §9 — coordinate before editing.
  4. `src/Hackmum.Bethuya.Backend/Program.cs` — register `CommunitySimulationSeeder` via `AddScoped<>()`.
  5. `tests/Hackmum.Bethuya.Tests/Development/CommunitySimulationSeederTests.cs` (or `Services/` to match existing convention) — unit/characterization tests: fixture determinism, all six persona anchors present, catalog not duplicated, idempotent rerun (in-memory DbContext), preserves `CurationSampleSeeder` behavior (regression check import only, no edits).
  6. `tests/Bethuya.IntegrationTests/CommunitySimulationSeederFlowTests.cs` — real Aspire/Postgres integration test: seed twice → identical logical counts/stable persona identifiers; verify via `GET /api/community/passport/journey` (as each persona, using the existing `X-Bethuya-Dev-Persona` header) and `GET /api/community/passport/dashboard/read-model` (RequireOrganizer, as Vikram) that seeded data is visible through existing APIs — not just direct table checks; verify Farah (Attendee-only) gets 403 and Vikram (Organizer) succeeds on the new seed endpoint; verify endpoint absent/fails closed outside Development.
- **Constraints carried from parent prompt:** deterministic natural fixture keys/provenance markers (e.g. `ProvenanceKey = "sim-seed:{personaKey}:{ordinal}"`) over raw Guid literals; Vogen IDs only (no raw Guid/int primitives); EF retry-safe `Database.CreateExecutionStrategy().ExecuteAsync(...)` wrapping `BeginTransactionAsync` (mirror `ParticipationLedgerService.WriteAsync`); small reviewable dataset (6 personas × a few ledger/registration rows) — no Bogus, no 1000-member generation (that's optional Layer 6); repair-only-safe-missing-fixtures semantics on rerun (never overwrite arbitrary user data).
- **Acceptance:** All six persona anchors persisted & locatable by stable external key; rerun produces identical logical counts (no duplicate members/identities/ledger provenance); ledger uniqueness constraint never violated; unauthorized persona (Farah) → 403, authorized (Vikram) → success; endpoint fails closed outside Development; `seed-curation`/`/api/dev/curation/seed` untouched and still passing its existing tests; targeted TUnit unit tests + real `dotnet test tests/Bethuya.IntegrationTests` (Aspire/Docker/Postgres) + full `dotnet build Bethuya.slnx` all green; code-review + dotnet-diag:optimizing-dotnet-performance findings resolved; PR opened targeting `indcoder-developer-persona-toolbar` (PR #53), not main. Layers 5/6 explicitly out of scope for this PR.
...
- **Status:** done
- **Agent/Owner:** Tank (implementation + perf fix ✅), Morpheus (security review — ✅ approved with 2 fixes: CSRF drive-by guard + open-redirect guard, plus a self-found defense-in-depth env guard), Neo (architecture review — ✅ approved, 8/8 checklist items pass), Switch (integration test — ✅ written and independently executed 9/9 by coordinator), code-review built-in (✅ 2 findings, both fixed by Morpheus), dotnet-perf built-in (✅ 1 moderate finding fixed by Tank, 1 info-level FrozenDictionary note deemed not worth applying)
...
10. ✅ TUnit tests — `DevelopmentPersonaSwitchingTests.cs` (26 tests, incl. 7 added by Morpheus for CSRF/open-redirect) + `DevelopmentEndpointGuardTests.cs` (3 tests) covering catalog, principal shape, three-way resolution, Farah-vs-Vikram policy proof, env/provider isolation, StateProvider per-request, PropagationHandler, CSRF guard, open-redirect guard, and Production/Staging/Development env-guard behavior. All 321 unit tests pass (0 regressions).
11. ✅ `.squad/decisions/inbox/` — `tank-persona-fail-closed-default.md`, `morpheus-persona-security-review.md`, `neo-persona-architecture-review.md`, `switch-persona-integration-test.md`, `tank-perf-ioptions-fix.md` — decision trail recorded for Scribe merge
12. ✅ `docs/development-authentication.md` — "Superseded by Layer 2" note added and re-confirmed accurate after all review fixes
13. ✅ Full Aspire+Postgres integration test — `tests/Bethuya.IntegrationTests/DevelopmentPersonaSwitchingFlowTests.cs` (5 tests: Farah→403, Vikram→auth-gate 404, Vikram decision→DecidedBy=vikram@bethuya.dev, Farah/Vikram identity diagnostics). **Independently executed by coordinator against live Docker/Aspire/Postgres containers: 9/9 passed** (2 full runs, ~52-61s each). Docker was confirmed available (v29.6.2 client, Desktop 4.84.0 server) — this is real execution evidence, not build-only.
14. ✅ Morpheus security review — APPROVED WITH FIXES. Round 1: found+fixed missing defense-in-depth env guard on `MapDevelopmentEndpoints` (commit `95b7234`). Round 2 (after coordinator relayed code-review's findings): fixed CSRF drive-by cookie-plant vulnerability (`Sec-Fetch-Site` guard → 403) and open-redirect via `returnUrl` (`IsLocalUrl()` + `Results.LocalRedirect()`) in commit `181ef4a`, adding 7 new tests. Neo architecture review — APPROVED, 8/8 checklist items pass, no blocking findings. code-review (built-in) — found the 2 defects above, both fixed by Morpheus (author Tank locked out per reviewer-rejection protocol). dotnet-perf (built-in) — 1 moderate finding (per-request `IConfiguration` string-parse instead of `IOptions<T>`) fixed by Tank in commit `95f6e6e`; 1 info-level `FrozenDictionary` note for the 6-entry catalog deemed not worth applying (reviewer's own assessment).

**`DecidedBy` path:** `CurationEndpoints.cs` line ~136 already derives `DecidedBy` from `user.FindFirst("email")`. Once the Backend authenticates as a named persona, `DecidedBy` automatically reflects the persona email (e.g. `vikram@bethuya.dev`). No changes to `CurationEndpoints.cs` required — confirmed by code trace, and confirmed empirically by the integration test's Decision assertion.

**Build status:** ServiceDefaults ✅ · Bethuya.Hybrid.Web ✅ · Hackmum.Bethuya.Backend ✅ · Hackmum.Bethuya.Tests ✅ (321/321, 0 failures, 0 warnings) · Bethuya.IntegrationTests ✅ (9/9, real Aspire+Postgres execution)

**Exit-gate confirmation:** ✅ Persona changes observable in Backend auth (`/api/dev/identity`) · ✅ policy decisions differ Farah (403) vs Vikram (success) · ✅ Refit carries selected key across all 7 clients (Event, ImageUpload, PlanningCycle, Profile, CommunityPassport, Registration, Curation) · ✅ structured logs identify selected subject (no sensitive data) · ✅ a Decision record (`DecidedBy`) identifies the selected persona · ✅ no switched call executes as fixed `dev-user-001`/`dev@bethuya.local` · ✅ mechanism inert outside Development+Provider=None (guarded at endpoint mapping, handler resolution, and propagation handler). PR #52 opened targeting `indcoder-developer-auth-review` (not main).

## [2026-08-04] Developer Testing Harness — Layer 3: Developer Persona Toolbar (PR3)
- **Status:** done
- **Agent/Owner:** Trinity (frontend implementation), Switch (bUnit + E2E), Morpheus (security review), Neo (architecture review), code-review + dotnet-perf built-ins
- **Description:** Add the minimal development persona toolbar on top of the already-proven Layer 2 switching infrastructure. `DevPersonaToolbar.razor` in `Bethuya.Hybrid.Web/Components/Dev/` (Web-only, not shared RCL), rendering all 6 allowlisted personas from `DevelopmentPersonaCatalog` (no duplication), showing the active display name/email/subject/effective auth roles via `<AuthorizeView>`, and driving persona selection exclusively through the existing Layer 2 secure endpoint (`GET /dev/persona/{key}?returnUrl=`) via a native `<button>` calling `NavigationManager.NavigateTo(url, forceLoad: true)` (full server reload, existing CSRF + local-redirect validation — no cookie planting, no bypass; a plain `<a href>` was tried first but is intercepted by Blazor's enhanced navigation and never reaches the server — see lessons.md). Visibility is gated by a DI marker (`IDevPersonaToolbarFeature`) registered only when `Authentication:Provider=None && Environment=Development` (mirrors the exact condition used to register the Layer 2 endpoints), not by a cosmetic Razor `@if` alone. Wired into `Components/App.razor` as its own `@rendermode="InteractiveServer"` island. Explicitly excludes: simulation seeding, persona signals as local state, manual role override, quick actions, destructive reset (no clear/reset button), new roles, v2 domains.
- **Acceptance:** `DevPersonaToolbar.razor` renders `data-test="dev-persona-toolbar"`, `persona-selector`, `persona-{anish,priya,rohan,maya,farah,vikram}`, `active-persona`, `active-persona-role` on native wrapper elements only (never directly on Bb components); bUnit `DevPersonaToolbarRenderTests.cs` covers all 6 personas, active identity/roles, visibility (incl. negative case for Production/real-provider), and BB unknown-parameter runtime safety — 5/5 pass; E2E test (`DevPersonaAuthorizationFlowTests.cs`) switches to Farah via the toolbar, confirms hard-reload persistence via the SSR-rendered identity, confirms the organizer-gated `/curation/{eventId}` page returns HTTP 403 for Farah (a top-level `[Authorize]`-decorated Blazor Server page is denied at the ASP.NET Core middleware tier, not via client-rendered "not authorized" text — see lessons.md), then switches to Vikram and confirms HTTP 200 plus the expected data-loading business error, with 4 Playwright screenshots captured to `artifacts/`; targeted TUnit/bUnit + E2E + solution build pass; PR opened targeting `indcoder-developer-identity-switching` (PR #52), not main.
