...
## [2026-07-01] PR5 — Authorization audit + metrics + tracing (observability)
- **Status:** done
- **Agent/Owner:** Copilot CLI (stacked on merged PR1–PR4 base `indcoder-authz-architecture-review`)
- **Description:** Make every security-sensitive authorization decision observable & auditable **without logging PII**. Additive only — no change to PR3 default-deny/role-policy wiring or PR4 ownership semantics. (1) **Audit:** structured `ILogger` + `Meter` + `ActivitySource` through the existing OpenTelemetry pipeline (no DB audit table → no EF migration). Non-PII subject = `SubjectHash` (hex of `SHA256(sub)[..12]`); immutable `AuthorizationAuditEvent` holds only non-PII fields (Decision Allow/Deny/Bypass, PolicyName=BethuyaPolicyNames constant, ResourceType, ResourceId, SubjectHash, Route, OutcomeStatusCode, Timestamp). Records: registration IDOR denials (GET/{id}, gov-id, DELETE) + event-scoped list, role-bypass grants, decider-identity (curation/approval/planning approve+publish), onboarding-guard blocks, create-time stamping (Registration.UserId, Event.CreatedBy). Never log Email/Name/gov-id. (2) **Metrics:** `AuthorizationMetrics` Meter `Bethuya.Authorization` — `authorization.decisions{policy,decision,resource_type}` + `authorization.outcomes{route_group,status_code}` (401 vs 403). (3) **Tracing:** dedicated `ActivitySource` `Bethuya.Authorization` child span + non-PII tags; both wired into `ConfigureOpenTelemetry` (AddMeter/AddSource). New `ServiceDefaults/Auth/Observability/*`: enum, event record, SubjectHash, AuthorizationMetrics, AuthorizationActivity, IAuthorizationAuditor+impl, RecordResourceOwnership ext, OwnershipBypass (DRY refactor of ResourceOwnerHandler, kept parameterless), BethuyaAuthorizationAuditMiddlewareResultHandler (delegates to default → behavior unchanged). Auditor+metrics TryAdd-registered in `AddBethuyaAuthorization`; middleware handler registered Backend-only via `AddBethuyaAuthorizationObservability` (no Blazor impact).
- **Acceptance:** Failing-first TUnit: `SubjectHashTests`, `AuthorizationMetricsTests`, `AuthorizationAuditorTests`, `RegistrationOwnershipAuditTests` (exactly-one Deny record + Bypass + Allow; subjectHash==hash(sub); no PII), `AuthAuditNoPiiTests` (real auditor + captured logger/ActivityListener; sentinel email/name absent from logs+span tags), `AuthOutcomeMetricsTests` (anon→401, wrong-role→403 by route group). `dotnet build` 0 warnings/0 errors; full `dotnet test tests/Hackmum.Bethuya.Tests` green. No EF migration. Aspire boots (build-level).

- **Status:** done
- **Agent/Owner:** Copilot CLI (stacked on merged PR1/PR2/PR3 base `indcoder-authz-architecture-review`)
- **Description:** Close horizontal privilege escalation / IDOR. (1) Add server-authoritative ownership fields: `Registration.UserId` (`string?`) set server-side from `IUserContext` at create (never request body; 401 if unresolvable), and make `Event.CreatedBy` server-set from the validated principal (ignore spoofable body). EF migration `AddRegistrationUserId` (nullable column + index); existing rows backfill to NULL (fail-closed); `CurationSampleSeeder` stamps coherent `UserId`. (2) Resource-based authorization in ServiceDefaults.Auth: `BethuyaPolicyNames.ResourceOwner`, `SameOwnerRequirement`, decoupled `ResourceOwnerContext(string? OwnerUserId)`, `ResourceOwnerHandler` (bypass Admin/Organizer/Curator else subject==owner), wired in `AddBethuyaAuthorization`. (3) Per-endpoint ownership checks via `IAuthorizationService.AuthorizeAsync`: registration `GET/{id}`, `POST /{id}/government-id`, `DELETE /{id}` → 404 on ownership failure (existence-hiding); per-event registrations list restricted to bypass roles (attendee → 403). Events stay role-scoped (organizer/admin bypass — documented). No audit/OTel (PR5); no PR3 wiring changes beyond the new policy; no new providers; matched existing raw-`Guid` ID convention (no Vogen refactor).
- **Acceptance:** Failing-first TUnit: `ResourceOwnerHandlerTests` (owner/non-owner/bypass/null-owner), `RegistrationOwnershipTests` (IDOR positives/negatives, organizer bypass, gov-id, delete, event-list 403/200), `RegistrationCreateOwnershipTests` + `EventCreateOwnershipTests` (server-set fields, spoofed body ignored, 401 no-identity). `dotnet build` 0 warnings/0 errors; full `dotnet test tests/Hackmum.Bethuya.Tests` green; `dotnet ef migrations add` + snapshot consistent; Aspire boots + MigrationService applies cleanly.
- **Result:** ✅ `ResourceOwner` policy + `SameOwnerRequirement` + `ResourceOwnerContext` + `ResourceOwnerHandler` added to ServiceDefaults.Auth and wired via `AddBethuyaAuthorization` (handler registered idempotently with `TryAddEnumerable`). ✅ `Registration.UserId` server-set from `IUserContext` (401 if unresolvable); `Event.CreatedBy` server-set from validated principal, spoofed body ignored. ✅ Migration `20260630205632_AddRegistrationUserId` (nullable column + index, NULL backfill); snapshot has no pending changes; idempotent script emits valid DDL. ✅ Registration `GET/{id}`, `government-id`, `DELETE/{id}` enforce ownership → 404 on failure; event-scoped list restricted to bypass roles → 403 for bare attendee. ✅ Government-id endpoint `.DisableAntiforgery()` (bearer multipart API; backend has no antiforgery middleware). ✅ Full solution builds 0 warnings/0 errors; full suite 300/300 passes. ⚠️ Live Aspire/MigrationService DB apply not run in this environment (Docker registry pull blocked); migration verified via compile + idempotent script + snapshot consistency.
